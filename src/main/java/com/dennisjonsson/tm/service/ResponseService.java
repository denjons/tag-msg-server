...
public class ResponseService extends DataSource {
    ...
    public ArrayList<ResponseDTO> getResponsesForRequest(ResponseUpdateDTO responseUpdateDTO)
	    throws CSTServiceException {
	...
	storedProcedure.setParameter(0, responseUpdateDTO.request).setParameter(1, responseUpdateDTO.limit)
	...
	res.stream().map((resp) -> ResponseTransformer.toResponseDTO(resp, responseUpdateDTO.request))
		.forEach(dtos::add);
